...
def get_machine_identifier (conn, spconfig, hostname):
    ...
    try:
        ...
        spidCommand = "select spid from sensorprotocolconfig where id = {};".format(spconfig)
        idCur.execute(spidCommand)

        if idCur.rowcount > 1:
            logging.error("Too many rows returned when retrieving spid for id {}, expected only one row.".format(spconfig))
            return -1
        elif idCur.rowcount is 0:
            logging.error("No results found for spid with id {}, unable to retrieve machine identifier.".format(spconfig))
            return -1
        elif idCur.rowcount is -1:
            logging.error("Error trying to retrieve spid for id {}, unable to retrieve machine identifier.".format(spconfig))
            return -1

        spid = idCur.fetchone()[0]
        logging.debug("Executed sql: {} Retrieved spid = {}".format(spidCommand, spid))
        ...
        if spid in idmap:
            ...
            if idCur.rowcount > 1:
                logging.error("Too many rows returned when retrieving machine identifier for id {} and parameter name {}.".format(spconfig, paramName))
                return -1
            elif idCur.rowcount is 0:
                logging.error("No results found for machine identifer with id {} and parameter name {}.".format(spconfig, paramName))
                return -1
            elif idCur.rowcount is -1:
                logging.error("Error trying to retrieve machine identifier for id {} and parameter name {}.".format(spconfig, paramName))
                return -1

            machineId = idCur.fetchone()[0]
            logging.debug("Executed sql: {} Retrieved machineId = {}".format(paramCommand, machineId))
...
def multi_domain_count():
    ...
    for ip, lsList in deviceMap.items():
        if ip in multiDomainIPs:
            ...
            logging.debug("IP {} is associated with {} domains".format(ip, len(domainList)))
...
def get_multiple_domains(conn, ls):
    ...
    logging.debug("Attempting to start event search through API with URL: {}".format(searchURL))
    try:
        if usePassword:
            searchResponse = requests.post(searchURL, headers=jsonHeader, params=searchParams, auth=auth)
        elif useToken:
            searchResponse = requests.post(searchURL, headers=jsonTokenHeader, params=searchParams)
    except Exception as ex:
        logging.debug("Error executing API call {}".format(searchResponse.text))
        logging.debug(ex)
        return []
    ...
    if "search_id" in searchResponse.json():
        ...
        logging.debug("Initiated event search for log source {}, search id is {}".format(ls.sensordeviceid, searchId))
    ...
    if searchId is "":
        if searchResponse.status_code == 401:
            ...
            errStr = "Error: API returned code {}\n{}".format(searchResponse.staus_code, searchResponse.text)
            sys.exit(errStr)
    ...
    checks = 1
    while checks < 60:
        logging.debug("Check if search is complete with URL {}".format(statusURL))
        logging.debug("Attempt {}".format(checks))
        statusResponse = None

        try:
            if usePassword:
                statusResponse = requests.get(statusURL, headers=jsonHeader, auth=auth).json()
            elif useToken:
                statusResponse = requests.get(statusURL, headers=jsonTokenHeader).json()

            logging.debug("Search status is {}".format(statusResponse["status"]))
            if statusResponse["status"] == "COMPLETED":
                logging.debug("Event search is complete for log source {}".format(ls.sensordeviceid))
                searchComplete = True
                break
    ...
    jsonRangeTokenHeader = {'SEC': token, 'Range': 'items=0-49', 'Accept': 'application/json'}

    eventsURL = 'https://{}/api/ariel/searches/{}/results'.format(consoleIP, searchId)
    ...
    try:
        if usePassword:
            eventsResponse = requests.get(eventsURL, headers=jsonRangeHeader, auth=auth)
            eventsResultList = eventsResponse.json()
            logging.debug("results: {}".format(eventsResponse.text))
        elif useToken:
            eventsResponse = requests.get(eventsURL, headers=jsonRangeTokenHeader)
            eventsResultList = eventsResponse.json()
    except Exception as e:
        logging.error("Error retrieving results for events search for log source {}".format(ls.sensordeviceid))
        logging.error(e)
        return []

    if not eventsResponse.ok:
        logging.error("API call was unsuccessful: {}".format(eventsResponse.text))
        logging.error("Couldn't retrieve domains for log source {}".format(ls.sensordeviceid))
        return []

    logging.debug("API call was successful to retrieve event search results for log source {}".format(ls.sensordeviceid))
    for jsonData in eventsResultList["events"]:
        if "domainid" in jsonData:
            domainId = jsonData["domainid"]
            logging.debug("Found domainid {} for log source {}".format(domainId, ls.sensordeviceid))
            try:
                domNameCur = conn.cursor()
                domNameCommand = 'select name from domains where id = {};'.format(domainId)
                ...
                if domNameCur.rowcount is 1:
                    ...
                    logging.debug("Domain id {} is for domain {}".format(domainId, domainName))
                    domains.append(domainName)
                else:
                    logging.error('Error retrieving domain name for id {}.'.format(domainId))
...
def set_domain(conn, ls):
    try:
        ...
        if domainCur.rowcount is -1:
            logging.error("Error trying to retrieve domain for log source {}.".format(ls.sensordeviceid))
            return
        elif domainCur.rowcount is 0:
            if multidomain:
                # Log source may be associated with multiple domains
                logging.debug("Log source {} may have multiple domains".format(ls.sensordeviceid))
            ...
            logging.debug("Log source {} has one domain".format(ls.sensordeviceid))
            domain = domainCur.fetchone()[0]
            ls.domain.append(domain)
            logging.debug("Domain for Log source {} was set to {}".format(ls.sensordeviceid, domain))

    except Exception as e:
        logging.error('Error retrieving domain for log source {}'.format(ls.sensordeviceid))
        logging.error(e)
        return
...
try:
    ...
    cur.execute("select id, hostname, devicename, devicetypeid, spconfig, timestamp_last_seen from sensordevice"
    ...
    for row in cur.fetchall():
        ...
        machine = get_machine_identifier(conn, device.spconfig, device.hostname)
        ...
        if machine in deviceMap:
            ...
            logging.debug("Machine {} in the map, adding device with id {} to its list".format(machine, device.sensordeviceid))
        else:
...
for identifier in deviceMap:
    try:
        ...
        logging.error(e)
        logging.error("Unable to to resolve hostname {} to IP".format(identifier))
        continue
